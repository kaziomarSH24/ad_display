...
<x-app-layout>
    <div class="pt-[5rem] overflow-x-hidden">
        ...
        <div class="bg-[#FFFFFF] rounded-[12px] pt-3 pb-10 mb-10 px-4">
            <h3 class="text-[#0D2927] f-plus font-[500] text-[24px]">Edit Quiz</h3>
                ...
                <div class="lg:w-[35rem] mx-auto">
                    <form method="post" action="{{ route('quiz.update', $quiz->id) }}" enctype="multipart/form-data">
                        ...
                        <div class="f-inter flex flex-col mb-6">
                            <label class="f-inter text-[16px] text-[#0D2927] mb-1 font-bold">Quiz No.</label>
                            <x-text-input name="quiz_no" type="number" class="mt-1 block w-full"
                                :value="old('quiz_no', $quiz->quiz_no)" required />
                        </div>

                        <div class="f-inter flex flex-col mb-6">
                            <label class="f-inter text-[16px] text-[#0D2927] mb-1 font-bold">Question</label>
                            <textarea name="question" required
                                class="bg-[#EEF6F1] border border-[#F3FFF2] py-4 px-3 rounded-[5px]">{{ old('question', $quiz->question) }}</textarea>
                        </div>

                        <div class="f-inter flex flex-col mb-6">
                            <label class="f-inter text-[16px] text-[#0D2927] mb-1 font-bold">Image</label>
                            <input name="image" type="file"
                                class="bg-[#EEF6F1] border border-[#F3FFF2] py-4 px-3 rounded-[5px] w-full">
                            @if($quiz->image)
                            <img src="{{ asset($quiz->image) }}" width="100px" class="mt-2 rounded">
                            @endif
                        </div>
                        ...
                        {{-- <div class="flex justify-between items-center mb-2">
                            <h3 class="text-lg font-bold text-[#0D2927]">Answers</h3>
                            <button id="addnew" type="button" class="p-2">
                        ...
                        <div id="ans" class="mb-6">
                            @foreach ($quizAns as $index => $quizAn)
                            <div class="ansdiv">
                                <label class="f-inter mt-5 text-[16px] text-[#0D2927] mb-1 font-bold">Answer</label>
                                <div class="flex w-full items-center">
                                    <input required type="text" name="answers[]"
                                        value="{{ old('answers.' . $index, $quizAn->ans) }}"
                                        class="bg-white flex-grow border border-[#a5a7a5] py-4 px-3 rounded-[5px]">

                                    <div class="flex items-center ml-4">
                                        <input required type="radio" id="is_correct_{{ $index }}" name="is_correct"
                                            value="{{ $index }}"
                                            class="h-4 w-4 text-green-600 focus:ring-green-500 border-gray-300"
                                            @if($quizAn->is_correct) checked
                                        @endif
                                        >
                                        <label for="is_correct_{{ $index }}"
                                ...
                                <div class="mt-3 mb-3">
                                    ...
                                    @php
                                    $selectedTags = explode(',', $quizAn->advertisement_id);
                                    @endphp
                                    <select name="tag[{{ $index }}][]" required multiple class="tags-select w-full">
                                        @foreach ($advertiseMents as $advertiseMent)
                                        <option value="{{ $advertiseMent->id }}" {{ in_array($advertiseMent->id,
                                            old('tag.' . $index, $selectedTags)) ? 'selected' : '' }}>
                                            {{ $advertiseMent->tag }}
                                        </option>
                                        @endforeach
                                    </select>
                        ...
                        <div class="lg:w-[20rem] mx-auto mb-5 pb-5">
                            <button type="submit"
                                class="text-lg font-bold text-white bg-green-600 rounded-[4px] py-3 w-full">
                                Update Quiz
                            </button>
                        </div>
                    </form>
        ...
        $("#addnew").on('click', function() {
            ...
            <div class="ansdiv mt-3">
                <div class="flex w-full items-center">
                    ...
                    <button class="ml-4 minusdiv" type="button">
                        <svg style="width: 15px; height: 15px;" viewBox="0 0 448 512">
                ...
                <div class="mt-3">
                    <select name="tag[${answerIndex}][]" required multiple class="tags-select w-full">
                        ${options}
                    </select>
                </div>
            </div>`;
